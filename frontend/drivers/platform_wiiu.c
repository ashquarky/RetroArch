...
#include <../../wiiu/include/nn/ac/ra_ac.h>
...
int getBroadcastAddress(uint32_t* broadcast)
{
   uint32_t myIp, mySubnet;
   NNResult result;
...
{
#ifdef HAVE_LIBMOCHA
   int init_result = Mocha_InitLibrary();
   DEBUG_VAR(init_result);
   if (init_result >= 0)
      return true;
#endif /* HAVE_LIBMOCHA */
   return false;
...
{
   int usb_result, fat_result;
   if (!iosuhaxMount)
      return;

#ifdef HAVE_LIBMOCHA
   usb_result = Mocha_MountFS("storage_usb", NULL, "/vol/storage_usb01");
   DEBUG_VAR(usb_result);
#ifdef HAVE_LIBFAT
   fat_result = fatInitDefault();
   DEBUG_VAR(fat_result);
#endif
#endif
}
